...
```bash
# Clone
git clone https://github.com/your-username/sakura-beta.git
cd sakura-beta
...
REDIS_URL=
BINANCE_API_KEY=
STACKS_DEPLOYER_KEY=      # keep this secret
...
Three tabs, cached in Redis, refreshed every 60 seconds:

1. **Top Agents** — ranked by prediction accuracy (min 10 predictions)
2. **Top Bettors** — ranked by total STX won
3. **Agent Creators** — ranked by total market volume generated
...
- **`@stacks/connect` v8** uses `request('stx_callContract', ...)` — not `openContractCall` from v7
- **`hexToCV`** expects `Uint8Array` — use `as any` cast when passing string from Hiro API response
- **`bytesToHex`** lives in `@stacks/common`, not `@stacks/transactions`
- **On-chain market ID** parsed from `tx_result.repr` (e.g. `(ok u12)`) to avoid race conditions with `get-market-count`
- **Nonce conflicts** — space out rapid Stacks transactions from the same address (`ConflictingNonceInMempool`)
- **Hiro API in Docker** — set `follow_redirects=True` on httpx client
- **RestrictedPython** strips standard builtins — `list`, `dict`, `len` must be explicitly re-added to sandbox globals

---

## Roadmap

- [ ] sBTC as bet currency
- [ ] Decentralized price oracle (Pyth on Stacks)
- [ ] Multi-asset support beyond BTC/ETH
- [ ] Agent marketplace — browse and fork public agents
- [ ] Passkey/WebAuthn support via `secp256r1-verify` on-chain
- [ ] Protocol governance contract for parameter updates
- [ ] Mobile UI
...
*Built on Stacks · Settled on Bitcoin · Powered by AI*
